...
//   }
  ...
  /**
   * Generează un token de serviciu pentru apelurile către alte microservicii
   */
  ...
  private async getEmployeesWorkingOnDate(departmentId: number, date: string): Promise<Array<{id: number, first_name: string, last_name: string}> | null> {
    try {
      ...
      const employeesResponse = await firstValueFrom(
        this.httpService.get(`http://giurom.bitap.ro:3002/employees?limit=10000`, {
          headers: {
            'x-internal-service': 'tasks',
            'x-service-secret': process.env.SERVICE_SECRET || 'default-service-secret',
            'Content-Type': 'application/json'
  ...
  private async getDepartmentEmployees(departmentGroupId: string): Promise<Array<{id: number, first_name: string, last_name: string}> | null> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get(`http://giurom.bitap.ro:3002/employees?department_id=${departmentId}&limit=10000`, {
          headers: {
            'x-internal-service': 'tasks',
            'x-service-secret': process.env.SERVICE_SECRET || 'default-service-secret',
            'Content-Type': 'application/json'
  ...
  private isTaskExpired(assignment: TaskAssignment, now: Date): boolean {
    ...
    if (finalizedInElement?.value) {
      try {
        ...
        try {
          // Încearcă să parseze ca JSON (format nou: {"hours":0,"minutes":3})
          const jsonData = JSON.parse(finalizedInValue);
          if (jsonData.hours !== undefined && jsonData.minutes !== undefined) {
            hours = jsonData.hours || 0;
            minutes = jsonData.minutes || 0;
            this.logger.log(`⏰ Task ${assignment.id} - Parsed finalized_in JSON: ${hours}h ${minutes}m`);
          } else {
            throw new Error('Not JSON format');
          }
        } catch (e) {
          // Fallback la formatul vechi (string: "2:30" sau "2")
          if (finalizedInValue.includes(':')) {
            // Format "2:30" - ore:minute
            const parts = finalizedInValue.split(':');
            hours = parseInt(parts[0]) || 0;
            minutes = parseInt(parts[1]) || 0;
            this.logger.log(`⏰ Task ${assignment.id} - Parsed finalized_in string: ${hours}h ${minutes}m`);
          } else {
            // Format "2" - doar ore
            hours = parseInt(finalizedInValue) || 0;
            this.logger.log(`⏰ Task ${assignment.id} - Parsed finalized_in hours only: ${hours}h`);
          }
        }
        ...
        this.logger.log(`⏰ Task ${assignment.id} deadline check: now=${this.formatDateForRomania(now)}, deadline=${this.formatDateForRomania(deadline)}, expired=${now > deadline}`);
  ...
  /**
   * Calculează noul deadline după reatribuirea unui task
   */
  private calculateNewDeadlineAfterReassignment(assignment: TaskAssignment, reassignmentTime: Date): Date | null {
    try {
      // Caută elementul finalized_in pentru deadline calculat
      const finalizedInElement = assignment.elements?.find(el => 
        el.task_element?.element_type === 'finalized_in'
      ...
      if (finalizedInElement?.value) {
        ...
        // Calculează noul deadline: momentul reatribuirii + finalized_in
        const newDeadline = new Date(reassignmentTime.getTime() + (hours * 60 + minutes) * 60 * 1000);
        
        this.logger.log(`⏰ Task ${assignment.id} - Noul deadline calculat: ${this.formatDateForRomania(reassignmentTime)} + ${hours}h ${minutes}m = ${this.formatDateForRomania(newDeadline)}`);
      ...
      if (finishAtElement?.value) {
        const deadline = new Date(finishAtElement.value);
        this.logger.log(`⏰ Task ${assignment.id} - Deadline fix din finish_at: ${deadline.toISOString()}`);
        return deadline;
      }

      // Dacă nu există finalized_in sau finish_at, nu se setează deadline
      this.logger.log(`⏰ Task ${assignment.id} - Nu există finalized_in sau finish_at, nu se setează deadline`);
      return null;
    } catch (e) {
      this.logger.warn(`Failed to calculate new deadline for task ${assignment.id}:`, e);
      return null;
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (employeeIds.length > 0) {
          ...
          const allEmployees = employeesResponse.data?.employees || [];
          this.logger.log(`📋 Total employees from service: ${allEmployees.length}`);
          this.logger.log(`📋 Employee IDs from shifts: [${employeeIds.join(', ')}]`);
          this.logger.log(`📋 All employee IDs: [${allEmployees.map((e: any) => e.id).join(', ')}]`);
          
          // Filtrează doar ID-urile care există în lista de angajați
          const validEmployeeIds = employeeIds.filter(id => allEmployees.some((emp: any) => emp.id === id));
          this.logger.log(`📋 Valid employee IDs (existing in employees list): [${validEmployeeIds.join(', ')}]`);
          
          if (validEmployeeIds.length === 0) {
            this.logger.warn(`⚠️ No valid employees found for shifts. All shift employee IDs are invalid: [${employeeIds.join(', ')}]`);
            return false;
          }
          
          availableEmployees = allEmployees.filter((emp: any) => validEmployeeIds.includes(emp.id));
      ...
      // Filtrează angajații: exclude angajatul curent și angajații din grupul manager
      const currentEmployeeId = assignment.assigned_to_id;
      const eligibleEmployees = availableEmployees.filter((emp: any) => {
        // Exclude angajatul curent
        if (emp.id === currentEmployeeId) {
          return false;
        }
        
        // Exclude angajații din grupul manager (department_id = 4)
        // Managerii au de obicei department_id = 4 în sistemul tău
        if (emp.department_id === 4) {
          this.logger.log(`🚫 Excluding manager employee ${emp.id} (${emp.first_name} ${emp.last_name}) from reassignment`);
          return false;
        }
      ...
      if (eligibleEmployees.length === 0) {
        ...
        this.logger.log(`📋 Total available employees: ${availableEmployees.length}, excluded: ${availableEmployees.length - eligibleEmployees.length} (current + managers)`);
        return false;
      }
      ...
      await this.assignmentRepository.update(assignment.id, {
        assigned_to_id: selectedEmployee.id,
        assigned_at: now, // Resetează data de atribuire pentru a recalcula deadline-ul
      ...
      // Calculează noul deadline și îl setează în due_date pentru claritate
      const newDeadline = this.calculateNewDeadlineAfterReassignment(assignment, now);
      if (newDeadline) {
        await this.assignmentRepository.update(assignment.id, {
          due_date: newDeadline
        });
        this.logger.log(`⏰ Task ${assignment.id} - Noul deadline după reatribuire: ${newDeadline.toISOString()}`);
      }
