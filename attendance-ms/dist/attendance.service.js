...
let AttendanceService = class AttendanceService {
    constructor(shiftRepository, presenceRepository, presenceInflexionRepository, notificationsClient, httpService) {
    ...
    async addEmployeePoints(employeeId, points, reason) {
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.post('http://giurom.bitap.ro:3002/tasks/executions/daily-points', {
    ...
    async createPresence(createPresenceDto) {
        ...
        if (check_in) {
            ...
            if (checkInTime <= maxAllowedTime) {
                await this.addEmployeePoints(shift.employee_id, 5, `Punctualitate - început program la timp (${checkInTime.toLocaleTimeString('ro-RO')})`);
            }
            else {
