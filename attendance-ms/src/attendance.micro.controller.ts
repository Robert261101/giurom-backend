...
export class AttendanceMicroController {
  ...
  findAllPresences(@Payload() payload: { page?: number; limit?: number; shift_id?: number; status?: string; start_date?: string; end_date?: string; work_location_id?: number }) {
    return this.service.findAllPresences(
      ...
      payload.start_date, 
      payload.end_date,
      payload.work_location_id?.toString()
    );
  }
