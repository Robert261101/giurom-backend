...
describe('Attendance permissions (e2e)', () => {
  let app: any;
  let jwt: JwtService;
  ...
  it('200 with attendance.read', async () => {
    if (!app) return;
    await request(app.getHttpServer()).get('/probe/read').set('Authorization', `Bearer ${sign(['attendance.read'])}`).expect(200);
