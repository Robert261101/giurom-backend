...
export class AttendanceService implements OnModuleInit {
  private readonly EMPLOYEES_SERVICE_URL = 'http://localhost:3012';
  ...
  // Helper method to validate employee exists via HTTP call
  private async validateEmployeeExists(employeeId: number): Promise<boolean> {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${this.EMPLOYEES_SERVICE_URL}/employees/${employeeId}`)
      );
      return response.status === 200;
    } catch (error) {
      console.error(`Employee validation failed for ID ${employeeId}:`, error.message);
      return false;
    }
  }
  ...
  async createShift(createShiftDto: CreateShiftDto): Promise<Shift> {
    ...
    // Validare angajat prin HTTP call
    const employeeExists = await this.validateEmployeeExists(employee_id);
    if (!employeeExists) {
