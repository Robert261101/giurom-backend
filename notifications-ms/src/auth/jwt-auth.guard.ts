  ...
  canActivate(context: ExecutionContext) {
    ...
    if (context.getType() === 'rpc') {
      ...
      if (data?.bypassAuth || data?.headers?.bypassAuth) {
        this.logger.log('[JwtAuthGuard] Bypassing JWT authentication for internal service request (RPC)');
        return true;
      }
      ...
      this.logger.log('[JwtAuthGuard] No JWT auth possible in RPC context, allowing request');
      return true;
    }
    ...
    if (request?.bypassAuth) {
      this.logger.log('[JwtAuthGuard] Bypassing JWT authentication for internal service request (HTTP)');
      return true;
    }
    
    this.logger.log('[JwtAuthGuard] Proceeding with normal JWT authentication (HTTP)');
