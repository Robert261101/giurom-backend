 ...
 * formular. Dacă cele două rulează pe aceeași bază, împart aceeași tabelă — inofensiv, e doar
 * cache împrospătat din aceeași sursă.
 */
@Injectable()
export class Giurom2ZonesService {
  ...
  /**
   * Gestiunile valide pentru o locație, **doar din cache**, fără apel la App2.
   *
   * Aruncarea se face și de pe telefon, uneori pe conexiuni proaste: validarea n-are voie să
   * adauge un drum în rețea pe calea formularului. Cache-ul e umplut de listarea care a produs
   * selectorul, cu câteva secunde înainte. Cache gol ⇒ set gol ⇒ eticheta se ignoră, iar
   * giurom 2.0 validează oricum la ingest și raportează în `unresolved_zones`.
   */
  async cachedZoneIdsForLocation(locationId: number): Promise<Set<number>> {
    if (!Number.isFinite(locationId)) return new Set();
    const rows = await this.zoneRepo.find({ where: { location_id: locationId } });
    return new Set(rows.map((z) => Number(z.external_zone_id)));
  }
   ...
   * deja setate pe acest serviciu pentru stoc sau deșeuri — o variabilă de mediu în minus de
  ...
  private resolveZonesUrl(): string | null {
    ...
    const known =
      this.configService.get<string>('GIUROM2_STOCK_SYNC_URL')?.trim() ||
