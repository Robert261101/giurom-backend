...
export class InternalServiceGuard implements CanActivate {
  ...
  canActivate(
    ...
    this.logger.log(`[InternalServiceGuard] Checking headers - Service: ${internalService}, Secret present: ${!!serviceSecret}`);
    ...
    if (internalService && serviceSecret) {
      ...
      this.logger.log(`[InternalServiceGuard] Expected secret: ${expectedSecret.substring(0, 5)}..., Provided secret: ${serviceSecret.substring(0, 5)}...`);
      ...
      if (serviceSecret === expectedSecret) {
        ...
        this.logger.log(`[InternalServiceGuard] ✅ Internal service request allowed for service: ${internalService}`);
        return true;
      } else {
        this.logger.warn(`[InternalServiceGuard] ❌ Invalid service secret provided for service: ${internalService}`);
    ...
    this.logger.log('[InternalServiceGuard] No internal service headers found, allowing other guards to handle request');
    return true;
  }
}
