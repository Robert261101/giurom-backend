...
let StockService = class StockService {
    constructor(productRepo, stockRepo, txRepo, wasteRecordRepo, categoryRepo) {
    ...
    async createTransaction(dto) {
        ...
        if (dto.target) {
            const existingTx = await this.txRepo.findOne({ where: { stock_id: stock.id, type: dto.type, target: dto.target } });
            if (existingTx) {
                return existingTx;
            }
        }
    ...
    async findProductsWithCategories() {
        return await this.productRepo.find({
            relations: ['categories'],
            order: { name: 'ASC' }
        });
    }
    async findProductWithCategories(id) {
    ...
    async assignCategoriesToProduct(productId, assignCategoryDto) {
        const product = await this.productRepo.findOne({
            where: { id: productId },
            relations: ['categories']
        });
