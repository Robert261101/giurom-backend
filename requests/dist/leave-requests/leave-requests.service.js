...
let LeaveRequestsService = LeaveRequestsService_1 = class LeaveRequestsService {
    constructor(leaveRequestRepo, httpService) {
        this.leaveRequestRepo = leaveRequestRepo;
        this.httpService = httpService;
        this.logger = new common_1.Logger(LeaveRequestsService_1.name);
    }
    onModuleInit() {
        this.notificationsClient = microservices_1.ClientProxyFactory.create({
    ...
    async create(dto, currentUserId) {
        ...
        try {
            const response = await (0, rxjs_1.firstValueFrom)(this.httpService.get(`http://localhost:3012/employees/${dto.employee_id}`));
            if (!response.data) {
                this.logger.error(`Employee with ID ${dto.employee_id} not found`);
                throw new common_1.NotFoundException('Angajatul nu a fost găsit');
            }
        }
        catch (error) {
        ...
        if (currentUserId && currentUserId !== dto.employee_id) {
            ...
            throw new common_1.ForbiddenException('Nu poți crea cereri de concediu pentru alți angajați');
        }
        ...
        const overlappingRequests = await this.leaveRequestRepo
            ...
            .andWhere('lr.status = :status', { status: leave_request_entity_1.LeaveStatus.APPROVED })
            .andWhere('(lr.start_datetime <= :endDate AND lr.end_datetime >= :startDate)', { startDate, endDate })
            .getCount();
        if (overlappingRequests > 0) {
            this.logger.error(`Overlapping leave requests found for employee ${dto.employee_id}`);
            throw new common_1.BadRequestException('Există deja o cerere de concediu aprobată în această perioadă');
        }
        const leaveRequest = this.leaveRequestRepo.create({
            ...
            duration_unit: dto.duration_unit || leave_request_entity_1.DurationUnit.DAYS,
        });
        ...
        try {
            await (0, rxjs_1.firstValueFrom)(this.notificationsClient.send({ cmd: 'labels.expiring-soon' }, {
    ...
    async findAll(filters, currentUserId) {
        ...
        if (filters.start_date && filters.end_date) {
            queryBuilder.andWhere('lr.start_datetime >= :startDate AND lr.end_datetime <= :endDate', { startDate: new Date(filters.start_date), endDate: new Date(filters.end_date) });
        }
    ...
    async findPending(currentUserId) {
        ...
        return this.findAll({ status: leave_request_entity_1.LeaveStatus.PENDING }, currentUserId);
    }
    async findOne(id, currentUserId) {
        ...
        if (!leaveRequest) {
            ...
            throw new common_1.NotFoundException('Cererea de concediu nu a fost găsită');
        }
        if (currentUserId && !this.isManager(currentUserId) && leaveRequest.employee_id !== currentUserId) {
            ...
            throw new common_1.ForbiddenException('Nu ai permisiunea să vezi această cerere de concediu');
        }
        return leaveRequest;
    }
    async updateStatus(id, dto, currentUserId) {
        ...
        if (currentUserId && !this.isManager(currentUserId)) {
            ...
            throw new common_1.ForbiddenException('Nu ai permisiunea să aprobi/respingi cereri de concediu');
        }
        ...
        this.logger.log(`CRITICAL ACTION: Leave request ${id} status changed from ${oldStatus} to ${dto.status} ` +
    ...
    async remove(id, currentUserId) {
        ...
        if (currentUserId && leaveRequest.employee_id !== currentUserId) {
            ...
            throw new common_1.ForbiddenException('Nu poți șterge cereri de concediu ale altor angajați');
        }
    ...
    async getEmployeeStats(employeeId, year) {
        ...
        const totalDaysApproved = approvedRequests.reduce((total, request) => {
            if (request.duration_unit === leave_request_entity_1.DurationUnit.DAYS) {
                return total + request.duration_in_days;
            }
            else {
                return total + (request.duration_in_hours / 8);
            }
        }, 0);
        const pendingRequests = await queryBuilder
            .clone()
            .andWhere('lr.status = :status', { status: leave_request_entity_1.LeaveStatus.PENDING })
            .getCount();
        const rejectedRequests = await queryBuilder
            .clone()
            .andWhere('lr.status = :status', { status: leave_request_entity_1.LeaveStatus.REJECTED })
            .getCount();
    ...
    async findRequestsForApproval(managerId) {
        ...
        return this.findAll({ status: leave_request_entity_1.LeaveStatus.PENDING }, managerId);
    }
};
exports.LeaveRequestsService = LeaveRequestsService;
exports.LeaveRequestsService = LeaveRequestsService = LeaveRequestsService_1 = __decorate([
