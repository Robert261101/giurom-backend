...
export class LeaveRequestsService implements OnModuleInit {
  ...
  async create(dto: CreateLeaveRequestDto, currentUserId?: number): Promise<LeaveRequest> {
    ...
    try {
      const response = await firstValueFrom(
        this.httpService.get(`http://localhost:3012/employees/${dto.employee_id}`, {
          headers: {
  ...
  async findAll(filters: FilterLeaveRequestsDto, currentUserId?: number): Promise<LeaveRequest[]> {
    ...
    // Filtrare pe perioada - include cererile care se suprapun cu perioada specificată
    if (filters.start_date && filters.end_date) {
      ...
      queryBuilder.andWhere('lr.end_datetime >= :startDate', { startDate: new Date(filters.start_date) });
    } else if (filters.end_date) {
      queryBuilder.andWhere('lr.start_datetime <= :endDate', { endDate: new Date(filters.end_date) });
    }
  ...
  async updateStatus(id: number, dto: UpdateLeaveRequestStatusDto, currentUserId?: number): Promise<LeaveRequest> {
    ...
    // Permitem angajaților să-și aprobe propriile cereri de concediu
  ...
  private async isManager(userId: number): Promise<boolean> {
    // Rolurile sunt decise în frontend; backend-ul nu mai restricționează pe baza rolului.
    // Lăsăm accesul deschis pentru listare/afisare (ex. colorare calendar).
    return true;
  }
