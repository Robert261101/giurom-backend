...
export class ShiftChangeRequestsService {
  ...
  async findAll(filters: FilterShiftChangeRequestsDto, currentUserId?: number): Promise<ShiftChangeRequest[]> {
    ...
    if (filters.start_date && filters.end_date) {
      ...
      queryBuilder.andWhere('scr.end_datetime >= :startDate', { startDate: new Date(filters.start_date) });
    } else if (filters.end_date) {
      queryBuilder.andWhere('scr.start_datetime <= :endDate', { endDate: new Date(filters.end_date) });
    }
