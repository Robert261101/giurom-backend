...
INSERT INTO giurombitap_company.company_subscriptions (company_id, plan_code, status, starts_at)
SELECT
  c.id,
  CASE
  ...
  'active',
  NOW(3)
FROM giurombitap_company.companies c
WHERE c.company_type = 'client'
  AND NOT EXISTS (
    SELECT 1 FROM giurombitap_company.company_subscriptions cs WHERE cs.company_id = c.id
  );
