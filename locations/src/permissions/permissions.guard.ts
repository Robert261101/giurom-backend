...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (request.bypassAuth) {
      this.logger.log(
        "Bypassing permissions check for internal service request",
      );
      return true;
    }
    ...
    this.logger.log(
      `🔍 [PermissionsGuard] Handler name: ${handlerName}, Controller: ${controller.name}`,
    );
    ...
    this.logger.log(
      `🔍 [PermissionsGuard] Handler permissions: ${JSON.stringify(handlerPermissions)}`,
    );
    this.logger.log(
      `🔍 [PermissionsGuard] Class permissions: ${JSON.stringify(classPermissions)}`,
    );
    this.logger.log(
      `🔍 [PermissionsGuard] Final required permissions (getAllAndOverride): ${JSON.stringify(requiredPermissions)}`,
    );
    ...
    this.logger.log(
      `🔍 [PermissionsGuard] Checking permissions for ${request.method} ${path}`,
    );
    this.logger.log(
      `🔍 [PermissionsGuard] Required permissions: ${JSON.stringify(requiredPermissions)}`,
    );
    this.logger.log(
      `🔍 [PermissionsGuard] User permissions count: ${user.permissions?.length || 0}`,
    );
    this.logger.log(
    ...
    if (hasAll) {
      this.logger.log(
        `✅ User has all required permissions: ${requiredPermissions.join(", ")}`,
      );
      return true;
    }
    ...
    if (isRevenueEndpoint) {
      ...
      this.logger.warn(
        `User missing required permissions for revenue endpoint: ${requiredPermissions.join(", ")}`,
      );
      throw new ForbiddenException("Permisiuni insuficiente");
    }

    // Extrage locationId din params, path sau URL (proxy poate trimite path diferit)
    let locationId = request.params?.id || request.params?.locationId;
    const pathToMatch = path || (request as any).url?.split("?")[0] || "";
    if (!locationId && pathToMatch) {
      const match = pathToMatch.match(/\/locations\/(\d+)/);
      if (match) {
        locationId = match[1];
      }
    }
    ...
    const employeeId =
      ...
      user.userId ??
      user.sub ??
      (user as any).id_employee;
    const userWorkLocationId = user.work_location_id;

    this.logger.log(
      `🔍 Checking location access - isGetRequest: ${isGetRequest}, locationId: ${locationId}, employeeId: ${employeeId}, work_location_id: ${userWorkLocationId}, work_location_default_id: ${user.work_location_default_id}, path: ${path}, params: ${JSON.stringify(request.params)}`,
    );
    ...
    if (isLocationsListRequest) {
      this.logger.log(
        `✅ Allowing GET /locations for authenticated user (employee ID ${employeeId}) - service will filter locations`,
      );
      return true;
    }

    // Angajați cu permisiuni "read_own" (ex. read_own_locations, assignment.read_own, execution.read_own)
    // pot citi o locație la care frontend-ul le dă acces (ex. dashboard angajat).
    ...
    if (
      ...
      employeeId &&
      hasAnyReadOwn &&
      requiredPermissions?.includes("locations.read")
    ) {
      this.logger.log(
        `✅ Allowing GET /locations/${locationId} for employee ${employeeId} (read_own permission)`,
      );
      return true;
    }
    ...
    if (isGetRequest && locationId && employeeId) {
      this.logger.log(
        `🔍 Checking access to location ${locationId} for employee ${employeeId}`,
      );
      ...
      if (workLocationId && String(locationId) === String(workLocationId)) {
        this.logger.log(
          `✅ User accessing own location via work_location_id/work_location_default_id (location ID ${workLocationId})`,
        );
        return true;
      }
      ...
      try {
        ...
        const hasAccess = employeeLocations.some((el: any) => {
          ...
          return (
            elLocationId === locationIdNum ||
            String(elLocationId) === String(locationId)
          );
        });

        if (hasAccess) {
          this.logger.log(
            `✅ User accessing own location via employees_locations (employee ID ${employeeId}, location ID ${locationId})`,
          );
          return true;
        } else {
          this.logger.warn(
            `❌ Location ${locationId} not found in employee ${employeeId} locations list`,
          );
        }
      } catch (error: any) {
        ...
        if (error.code === "ECONNREFUSED" || error.code === "ETIMEDOUT") {
          try {
            ...
            if (result && result.length > 0) {
              this.logger.log(
                `✅ Found location ${locationId} in employees_locations via direct DB query for employee ${employeeId}`,
              );
              return true;
            } else {
              this.logger.warn(
                `❌ Location ${locationId} not found in employees_locations for employee ${employeeId}`,
              );
            }
          } catch (dbError: any) {
            this.logger.error(
          ...
          this.logger.error(
            `❌ Failed to check employee location access: ${error.message} (code: ${error.code || "unknown"})`,
          );
        }
      }
    }
