...
export function assertJwtEmployeeIdConsistency(user?: LeaveAccessUser): void {
...
export function getCanonicalEmployeeId(user?: LeaveAccessUser): number | null {
...
export function isOperationalStaffUser(user?: LeaveAccessUser): boolean {
...
export function isLeaveAdminUser(user?: LeaveAccessUser): boolean {
  const permissions = Array.isArray(user?.permissions) ? user.permissions : [];
  return (
    permissions.includes('assignment.read_all') ||
    permissions.includes('assignment.read_company')
...
export function isFurnizorSupplierAdmin(user?: LeaveAccessUser): boolean {
  const permissions = Array.isArray(user?.permissions) ? user.permissions : [];
  const roles = normalizeRoles(user?.roles);
