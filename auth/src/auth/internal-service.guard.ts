    ...
    if (internalService && serviceSecret) {
      ...
      if (!expectedSecret) {
        throw new UnauthorizedException(
          'SERVICE_SECRET nu este configurat în variabilele de mediu',
        );
      }
      if (secretsMatch(String(serviceSecret), expectedSecret)) {
        request.internalService = internalService;
        request.bypassAuth = true;
        this.logger.log(
          `Internal service request allowed for service: ${internalService}`,
        );
        return true;
      }
      this.logger.warn(
        `Invalid service secret provided for service: ${internalService}`,
      );
