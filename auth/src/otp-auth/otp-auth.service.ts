...
export class OtpAuthService {
  ...
  async verifyOtp(identifier: string, otp: string): Promise<{ access_token: string; refresh_token: string }> {
    try {
      ...
      // Generează token-urile folosind TokenRotationService
  ...
  private async findUserByEmail(email: string): Promise<any> {
    try {
      ...
      const employee: { id: number; email: string; first_name: string; last_name: string; phone: string; profile_image: string | null; birth_date: string | null; department_default_id: number | null; work_location_default_id: number | null } | null = await this.usersService.findEmployeeByEmail(email);
      if (!employee) {
      ...
      const result = {
        ...
        department_id: employee.department_default_id || null,
        work_location_id: employee.work_location_default_id || null,
  ...
  private async findUserByPhone(phone: string): Promise<any> {
    try {
      ...
      // Găsește angajatul prin microserviciul employees
      const employee: { id: number; email: string; first_name: string; last_name: string; phone: string; profile_image: string | null; birth_date: string | null; department_default_id: number | null; work_location_default_id: number | null } | null = await this.usersService.findEmployeeByPhone(phone);
      if (!employee) {
