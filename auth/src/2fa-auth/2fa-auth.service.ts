...
export class TwoFactorAuthService {
  ...
  async step2Verify(userId: number, otp: string): Promise<{ access_token: string; refresh_token: string }> {
    try {
      ...
      try {
        ...
        const employeeData: { id: number; email: string; first_name: string; last_name: string; phone: string; profile_image: string | null; birth_date: string | null; department_default_id: number | null; work_location_default_id: number | null } | null = await this.usersService.findEmployeeByEmail(user.email);
  ...
  private async findUserByEmail(email: string): Promise<any> {
    try {
      ...
      const employeeData: { id: number; email: string; first_name: string; last_name: string; phone: string; profile_image: string | null; birth_date: string | null; department_default_id: number | null; work_location_default_id: number | null } | null = await this.usersService.findEmployeeByEmail(email);
      if (!employeeData) {
        return null;
      }
  ...
  private async findUserByPhone(phone: string): Promise<any> {
    try {
      ...
      const employeeData: { id: number; email: string; first_name: string; last_name: string; phone: string; profile_image: string | null; birth_date: string | null; department_default_id: number | null; work_location_default_id: number | null } | null = await this.usersService.findEmployeeByPhone(phone);
      if (!employeeData) {
        return null;
      }
